...
Since we are talking about autonomous drone, then we have to consider
...
In addition, to know the answers for these three major milestones we
...
Finally, know how to get there we used modified A\* algorithm, to create
a track that helps scanning the field.
...
>
> These constrains briefly can be like
...
> Finally, to really get a high precision location data, IMU or inertial
...
> receiver determines its position by finding the intersection of
...
> two satellites leads to two points of intersection that a man could be
...
> location.
>
> <img src="media/image4.png" style="width:4.75833in;height:2.76806in" /><img src="media/image5.png" style="width:2.50347in;height:1.83289in"
> alt="NEO 6M GPS module pin out" />

#### NEO-M8N GNSS module 

> We used Neo-M8N module to retrieve the position data from GNSS, note
...
>
> That is because GNSS module has concurrent reception for multiple GNSS
> like GPS, GLONASS, BeiDou, and Galileo.
>
> In addition, it is important to know that this GNSS sends NMEA message
> that we will know more about it in [<u>next
> section</u>](#nmea-sentence).

#### NMEA sentence 

> NMEA stands for National Marine Electronics Association, which is a
> standard protocol created by Navy so they can integrate different
> devices on board and communicate.
>
> <img src="media/image6.png" style="width:3.88403in;height:1.40972in" />Basically,
...
> feed \<CR\>\<LF\>.
>
> <img src="media/image7.png" style="width:6.675in;height:2.2915in" />

Figure 5 GGA sentence
...
>
> <img src="media/image8.png" style="width:3.72222in;height:3.49167in" />So,
...
> Maps API mainly gives us insights to the following methods:
...
> Routes API mainly gives us insights to the following methods:
...
> Environment API mainly gives us insights about the environmental data
...
of Google, that can be like Opensource Routing, Mapbox and others, but
we mainly chose google maps since it first supports maps for our region
and it has better insights with free subscription plan than the limited
...
5.  1.  
    ...
    1.  1.  **Destination geocoding**

<img src="media/image9.png" style="width:3.20417in;height:2.7in" />
...
So, our algorithm will be simply as shown in Figure 7
...
Using the geocode method without parsing returns raw dictionary with
several ids and linked lists, latitude and longitude will be found in
the id of name ‘geometry’ and inside this dictionary another dictionary
...
>
> Our requirements are that we want the drone to cover the whole field
> or as much as it can so that it detects as much plants as it could,
...
>
> <img src="media/image10.png" style="width:7.58819in;height:3.14306in" />These
> algorithms mainly consider the field as a grid as shown in Figure 62,
> with cells that are named Nodes, where each node has defined ID, and
> the transition path between a Node and its adjacent is named Edge,
> where every edge has a weight, they may all have same weights, these
> weights indicates a weighted value that adds extra cost to the
> traveling above the distance cost, like for example a node is
> surrounded by 3 nodes, they might be all same geometric distance away
> from this node, but not all of them are easily accessible this might
> be due to extra turn before heading to a node, you can imagine it as
> extra traffic way, so the more crowded has higher weight value.
>
...
> well as minimum latitude and longitude of each node, you can imagine
...
It is important to understand and specify the nature of relationship
...
<img src="media/image44.png" style="width:2.72812in;height:1.54505in" /><img src="media/image45.png" style="width:1.475in;height:1.72931in" /><img src="media/image46.png" style="width:1.93842in;height:1.71258in" /><img src="media/image47.png" style="width:1.70833in;height:1.75in" />*Table
1 Graph Theory*
...
> in our case, or by other words, the start point is not our goal to
...
> following it in the flow, so it is not symmetric relationship but a
> directed one, that leads to having an algorithm that solves Directed
> graphs. Also, we assumed that all the weights of edges are constant
...
> constrains and requirement, we have to assess the available algorithms
> based on those KPIs:
...
<img src="media/image52.png" style="width:4.51389in;height:2.81458in" />

> Big O notation briefly is a symbolism that relates the times of
> computation as function of the worst number of iterations of a
...
> element will take time t, repeating the same operation for nth
> elements ends up to have n times the time of one operation to be
> executed so execution time is function of n O(n).
>
...
> according to the weight of edges that also requires to be examined, so
> we will also generally loop over the edges weights.
>
...
algorithm that inspired other modified algorithms, those two are BFS and
DFS that we have previously discussed in ‎1.3, but however we have not
specified yet which approach of them is more relevant and why.
...
with big number of nodes, unlike BFS, that is more probable to find the
...
A\* algorithm, that solves the queue problem by using Priority queue
...
features in filed and navigate upon these features, however it will
require bigger space to handle images captured online.

###  Longest path algorithm

As we have seen in ‎6.3.1BFS and specifically the modified one which is
A\* algorithm is the most standing relative to our requirements so far.

But however, A\* is a shortest path finding algorithm, when we said at
first that our goal is to generate the longest path in field to scan it,
to do that let’s first understand the idea of A\*

<img src="media/image54.png" style="width:2.09236in;height:1.50833in" /><img src="media/image54.png" style="width:2.10486in;height:1.51736in" /><img src="media/image54.png" style="width:2.09236in;height:1.50833in" />

**LONGEST PATH**

<img src="media/image56.png" style="width:2.05556in;height:1.54792in" /><img src="media/image56.png" style="width:2.05556in;height:1.54792in" /><img src="media/image56.png" style="width:2.05556in;height:1.54792in" /><img src="media/image56.png" style="width:2.05556in;height:1.54792in" />

As you can notice from Figure 11 A\* Algorithm, this algorithm is mainly
about choosing the minimum weights sum aongst the adjacent nodes to
every node.
...
> heuristic function is calculated from distance formula called
...
> have same value length from the current node, but one of them might be
> much far from the end point We used this to generate the longest path
...
> style="width:3.72639in;height:2.58333in" />

## Geodetic to NED

> It is important to consider that the Control system is expecting from
...
> radius is radius of EARTH and theta is calculated from haversine law,
> as function of lat. and long.
...
> <img src="media/image60.png" style="width:3.15625in;height:1.94722in" />
>
> <img src="media/image62.png" style="width:7.38125in;height:1.13889in" />

### Universal Transverse Mercator

As we know Latitude, Longitude and Altitude coordinates are angular
quantities in the Geodetic Frame, that specifies location on the Earth,
...
cumulating values through time, that might not be a practical
computation over long time.

In Aeronautics the standard local frame is known as NED Frame, which
goes for North-East-Down, where North is the cartesian X-coordinate,
East is Y-coordinate, and Down is the negative Z-Coordinate, note that Z
...
Mercator (UTM), UTM divides the world’s 3-D sphere to a 2-D map with
neglecting the altitude, this map has 60 east-west numbered zones, and
24 north-south zones marked as letters.

In this part we are interested to calculate the distance the drone
...
defining the size of grid that we want, for example size=100 means we
will get a 100x100 meters far from the center latitude and center
longitude that we have chosen if the precision of step is 1 meter.

<img src="media/image65.png" style="width:2.71181in;height:2.11389in" />But
as we have mentioned earlier latitude and longitudes are degree units,
so we need to know the step from point to another, or the precision in
degrees not meters. To do that we should have a refresher on latitudes
and longitudes to calculate the distance between 2 degree of latitude,
...
infeasible points is as in the following pseudo code.
...
<img src="media/image67.png" style="width:1.35in;height:1.46806in" />

<img src="media/image69.jpeg"
style="width:6.38264in;height:3.34167in" /><img src="media/image70.png" style="width:6.38264in;height:3.34167in" /><img src="media/image76.png" style="width:3.36875in;height:3.52986in" /><img src="media/image70.png" style="width:3.17116in;height:3.56061in" />

There are many factors that changes the profile of the grid, starting
...
other object might be over scaled. That is why local path plan system is
...
are in same east-west line, then we could simply pass x and z only.

to know if 3 points are in same straight line or not, then in 2-d their
area should be zero, if their area is not zero then they are not in same
straight line, but they can form triangle.
...
then we can use this check to extract the way points from filtering
poly-points in same line.

The collinearity check system checks if P1, P2, and P3 are in same
straight line, then we will drop P2.

<img src="media/image78.png" style="width:3.775in;height:2.60208in" />

Figure 23 Longest path plan for grid

## Skeletonization

As we are planning for the drone movement, we have to put in
consideration the degrees of freedom or constraints that we are
controlling, such as the drone orientation, diagonal movement, 3-d
movement and grid size. So, adding all of these in the grid-based search
will be computationally expensive though grids assure that u find the
optimal and complete path according to its accuracy, unlike graphs that
doesn't represent the geometry with obstacles of plane but it represents
the topology of the map and the edges(curved/straight/diagonal) that
connects nodes (STATES) together.

Skeletonization in our path plan can be simply done by finding the
Medial Axis of the grid, which creates a less feasible points to visit,
where these points lies in the mid between each corresponding obstacle,
this drops other points that we have considered before such as the
points that almost touches the obstacles.

This consequently down size the feasible points that our A\* will search
for.

<img src="media/image79.png" style="width:3.69984in;height:2.76944in" />

Figure 24 Grid skeleton

<img src="media/image80.png" style="width:2.48403in;height:2.34167in" /><img src="media/image81.png" style="width:2.43264in;height:2.34167in" />

As we can observe from Figure 25 The Medial axis path covers a safe area
for the drone to fly over, with less points i.e. less way points, less
time and less memory. However, we have also downsized the accuracy here,
since the camera has the capability to capture wide view, but this might
not be optimum solution for high accuracy applications.
...
> meters space, this space if empty, the algorithm will probably get
> stuck in computation. And after skeletonization, the algorithm will
> work well, however, the size of feasible points the is passed to the
> algorithm are *n* variables with the start and end chosen points, then
...
> time, to loop over each 3 points to check if they are collinear and
...
> collinearity check.
>
> <img src="media/image82.png" style="width:2.675in;height:2.60747in" /><img src="media/image83.png" style="width:2.69236in;height:2.61667in" />To
> achieve that, we will create a Tree of the random points, and points
> here is geometric points i.e. have area, permitter and all the
> geometric properties using Shapely module, and then query the polygons
